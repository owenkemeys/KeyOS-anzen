...
fn run() -> Result<(), String> {
    ...
    if matches!(
        args.first().and_then(|value| value.to_str()),
        Some("add-recovery-friend")
    ) {
        return add_recovery_friend_command(&args);
    }
    if args.len() != 4 {
...
fn add_recovery_friend_command(args: &[std::ffi::OsString]) -> Result<(), String> {
    if args.len() != 6 {
        return Err("invalid command arguments".into());
    }
    ...
    let config = VaultConfig::parse_bounded(&read_bounded(Path::new(&args[2]))?)
        .map_err(|error| error.to_string())?;
    let public_key = read_bounded(Path::new(&args[3]))?;
    let output = Path::new(&args[4]);
    if output.exists() {
        return Err("output file already exists".into());
    }
    let seed_text = args[5]
        .to_str()
        .ok_or_else(|| "development seed is not UTF-8".to_owned())?;
    let mut seed = decode_seed(seed_text)?;
    let result = (|| {
        ...
        let reviewed = backup
            .review_friend_enrollment(config, &public_key)
            .map_err(|error| error.to_string())?;
        let fingerprint = reviewed.summary().fingerprint.clone();
        let current_friend_count = reviewed.summary().current_friend_count;
        ...
        atomic_write_new(
            output,
            &approved.to_json().map_err(|error| error.to_string())?,
        )?;
        println!("Validated and enrolled Anzen recovery friend");
        println!("OpenPGP fingerprint: {fingerprint}");
        println!("Vault address: {vault_address}");
        println!("Recovery friends: {}", current_friend_count + 1);
        Ok(())
    })();
    seed.fill(0);
    result
